...
count = 0
with open("TimesTotalList.txt", 'r', encoding='UTF-8') as f:
    for line in f:
        line = line.strip('\n')
        try:
            city, district, cinema_name, time_url = line.split(' ')
            # print(city, district, cinema_name, time_url)
            city = city.replace('市', '')
            ...
            if ratios and np.max(ratios) > 0.65:
                ...
                cinema_list[selected_index].time_url = time_url
